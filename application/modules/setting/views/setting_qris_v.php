   ...
   <button class="btn btn-primary waves-effect waves-light btn-sm" style="float:right;" onclick="getform('setting_content')" type="button"><i class="fa fa-Left"></i> Kembali</button>
    ...
    <table class="table table-striped table-bordered" id="table_form" width="100%">
        ...
        <tbody>
          <?php 
            ...
            echo '<tr>';
            echo '<td> Jumlah Data QRIS</td>';
            echo '<td id="data_qris" align="right">'. $data_qris[0]['agen_qris'] .'</td>';
            echo '<td align="right"><button class="btn btn-warning waves-effect waves-light btn-sm" style="float:right;" onclick="showform(\'qris\')" type="button"><i class="fa fa-plus"></i> edit</button></td>';
            echo '</tr>';
          ?>
        </tbody>
    </table>
    ...
    <script>
        ...
        function sendform(){
            ...
            var notif   = "Update Jumlah Data Qris Berhasil ";
            var address = "./setting/up_qriss";
            sendajax(data, address, null, notif, null);
            document.getElementById("data_qris").innerHTML=$('#value_data').val();
            $("#modal").hide();
        }
